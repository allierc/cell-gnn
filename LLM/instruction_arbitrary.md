# Cell-GNN Exploration: Robust g_phi Recovery on Arbitrary 3-Type Pair Forces

## Goal

Find GNN training configurations that **robustly recover the pairwise interaction function g_phi** across different random seeds and simulation regimes.
...
Note: more frames = more training data = longer training. Adjust `data_augmentation_loop` inversely.
...
- **Slot 3**: Explore — different data regime (delta_t, n_frames)
...
Config: hidden_dim=X, n_layers=X, lr=X, lr_emb=X, batch=X, aug_loop=X, delta_t=X, n_frames=X
...
```markdown
# Working Memory: arbitrary g_phi exploration
