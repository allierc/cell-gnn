...
@register_model("particle_spring_force_ode_static_field_mlp")
class CellGNNMLPField(nn.Module):
    """GNN for pair forces + MLP for learning the static vector field.

    The GNN branch (lin_edge) learns pairwise interactions exactly like CellGNN.
    The MLP branch (lin_field) learns the mapping pos -> field_velocity,
    replacing the ground-truth gradient sampling with a learned approximation.

    Total output: d_pos = GNN_pair(edges) + MLP_field(pos)
    """
    ...
    def __init__(self, config, device, aggr_type=None, bc_dpos=None, dimension=2):
        ...
        # --- MLP field branch: pos -> field velocity ---
        n_layers_field = getattr(model_config, 'n_layers_field', 5)
        hidden_dim_field = getattr(model_config, 'hidden_dim_field', 128)

        self.lin_field = MLP(input_size=self.dimension, output_size=self.dimension,
                             nlayers=n_layers_field, hidden_size=hidden_dim_field, device=self.device)
