...
class CellGNNSirenGradField(nn.Module):
    ...
    d_pos = GNN_pair(edges) + grad_x[ SIREN(x, y, z, t) ]

    The SIREN branch learns the **scalar** concentration field c(x, t).
    The chemotactic velocity is obtained as the spatial gradient of this
    scalar field via ``torch.autograd.grad``, which guarantees a
    curl-free (conservative) force field.

    This is more physically principled than directly learning a vector
    field, since the true chemotactic force is grad(c).
    """
    ...
    def __init__(self, config, device, aggr_type=None, bc_dpos=None, dimension=2):
        ...
        # --- SIREN field branch: (x, y, z, t) -> scalar c ---
        ...
        self.siren_field = Siren(
            ...
            out_features=1,                   # scalar concentration c
    ...
    def forward(self, state: CellState, edge_index: torch.Tensor, data_id=[], training=[], has_field=False, k=[]):
        ...
        # --- SIREN field branch: scalar c(x,t) -> grad_x(c) ---
        ...
        # Enable gradient tracking on spatial coordinates for autograd
        # Use enable_grad so this works even inside torch.no_grad() at inference
        with torch.enable_grad():
            pos_field = pos.detach().requires_grad_(True)
            siren_input = torch.cat([pos_field, t_norm], dim=1)  # (n_cells, dim+1)

            # Forward: scalar field c(x, t)
            c = self.siren_field(siren_input)  # (n_cells, 1)

            # Gradient: grad_x(c) via autograd — only w.r.t. spatial dims
            out_field = torch.autograd.grad(
                outputs=c,
                inputs=pos_field,
        ...
        # Store scalar field for diagnostics
        self.last_scalar_field = c.detach()
