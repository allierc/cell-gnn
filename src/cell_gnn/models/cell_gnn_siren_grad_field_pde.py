...
@register_model("particle_spring_force_diffusion_field_siren_grad_pde")
class CellGNNSirenGradFieldPDE(nn.Module):
    """Same as CellGNNSirenGradField, but also computes a PDE residual.

    Loss users should combine:
        L = |d_pos_pred - d_pos_target|^2   (position / velocity loss)
          + pde_weight * |R|^2              (diffusion-decay PDE residual)

    where R = dc/dt - D * laplacian(c) + lambda * c   (no source term)

    After each forward pass, ``self.last_pde_residual`` holds R (N, 1) and
    callers in the training loop can add its squared norm to the loss.

    D and lambda are read from config.simulation.field_params (keys
    ``diffusion_coeff`` and ``lambda_decay``).  Time is parameterised as
    ``t_phys = k * delta_t``, so dc/dt is in the same physical units as
    the generator's PDE.
    """
    ...
    def __init__(self, config, device, aggr_type=None, bc_dpos=None, dimension=2):
        ...
        # --- PDE coefficients (read from field_params) ---
        fp = getattr(simulation_config, 'field_params', {}) or {}
        # Support both dict-like and Namespace-like access
        ...
        # If enabled, the residual gets the known source term subtracted:
        #   R = dc/dt - D*Laplacian(c) + lambda*c - S_i(t)
        # where S_i(t) = alpha * gain * pulse_i(t) for emitting particles.
        ...
        # Lazily built on first forward (need n_cells and device):
        ...
        # --- Learnable saturating chemotaxis parameters ---
        # F_chem = mu_chem * log1p(|grad c| / g0) * grad c / |grad c|
        # Parameterise in log-space so the positivity constraint is automatic.
        init_mu = float(_fp_get('mu_chem', 0.1))
        init_g0 = _fp_get('chem_saturation_scale', None)
        # If saturation is disabled in the config, default g0 large so log1p(|g|/g0) ≈ |g|/g0,
        # which reduces to linear chemotaxis (mu_chem/g0 * grad c). The model can still
        # adapt via gradient descent.
        if init_g0 is None or init_g0 <= 0:
            init_g0 = 1.0
        self.log_mu_chem = nn.Parameter(torch.tensor(np.log(init_mu), dtype=torch.float32, device=self.device))
        self.log_chem_g0 = nn.Parameter(torch.tensor(np.log(init_g0), dtype=torch.float32, device=self.device))

        # Filled by forward(); trainer reads this to add to the loss.
    ...
    def forward(self, state: CellState, edge_index: torch.Tensor, data_id=[], training=[], has_field=False, k=[]):
        ...
        # --- SIREN scalar c(x,t): grad_x c + PDE residual via autograd ---
        ...
        with torch.enable_grad():
            ...
            # Laplacian(c) = sum_i d/dx_i (grad_c[:, i])
            ...
            # PDE residual: dc/dt - D * laplacian(c) + lambda * c [- S_i(t)]
            pde_residual = dc_dt - self.pde_D * laplacian_c + self.pde_lambda * c

            # Optional source term (known from config: alpha, mask, phases)
            if self.pde_source_aware and self.pde_alpha > 0:
                n_cells = pos.shape[0]
                if self._src_mask.numel() != n_cells:
                    # Rebuild mask + phases deterministically using the same seed
                    # as the data generator (matches the pulsed-source logic).
                ...
                if self.pde_pulse_period and self.pde_pulse_period > 0:
                    ...
                    source_rate = (self.pde_alpha * gain * pulse).unsqueeze(-1)  # (N, 1)
                else:
                    source_rate = (self.pde_alpha * self._src_mask.float()).unsqueeze(-1)

                pde_residual = pde_residual - source_rate

            # Chemotactic velocity = mu_chem * log1p(|grad c|/g0) * grad c / |grad c|
            # mu_chem and g0 are learnable (parameterised in log-space for positivity).
            mu_chem = torch.exp(self.log_mu_chem)
            g0 = torch.exp(self.log_chem_g0)
            g_norm = grad_c.norm(dim=1, keepdim=True).clamp_min(1e-12)
            g_hat = grad_c / g_norm
            out_field = mu_chem * torch.log1p(g_norm / g0) * g_hat

        # Stash for diagnostics + trainer to pick up
        self.last_scalar_field_detached = c.detach()
        self.last_pde_residual = pde_residual  # keep graph attached for backprop
        # Regularisation: penalise negative c and excessive field magnitude
        self.last_pde_reg = c  # keep graph attached
