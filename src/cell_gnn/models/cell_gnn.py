...
class CellGNN(nn.Module):
    ...
    def __init__(self, config, device, aggr_type=None, bc_dpos=None, dimension=2):
        ...
        match self.model:
            case 'arbitrary_ode' | 'particle_spring_force_ode' | 'particle_spring_force_dynamic_field':
    ...
    def psi(self, r, p1, p2=None):
        ...
        if self.model == 'gravity_ode':
            psi = p1 / r ** 2
            return psi[:, None]
        if self.model in ('particle_spring_force_ode', 'particle_spring_force_dynamic_field'):
