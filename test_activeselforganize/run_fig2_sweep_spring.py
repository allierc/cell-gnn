"""
Fig 2 phase-diagram-style sweep for communicating_activematter_spring.py.

Same v0 x omega grid as run_fig2_sweep.py — but using the soft-spring
repulsion variant of the simulation, so the dynamics are a smooth function
of state (suitable for GNN learning).

Per-run frames, npz, and mp4 are written into sweep_fig2_spring/<tag>/.
"""
...
import communicating_activematter_spring as sim
...
SWEEP_ROOT = "sweep_fig2_spring"
FRAMES_SUBDIR = "frames_spring"   # matches communicating_activematter_spring.run()
...
def run_one(args):
    ...
    try:
        ...
        frames_glob = os.path.join(out_dir, FRAMES_SUBDIR, "frame_*.png")
        cmd = [
...
def make_overview():
    ...
    for iy, omega in enumerate(reversed(OMEGA_VALUES)):
        for ix, v0 in enumerate(V0_VALUES):
            ax = axes[iy, ix]
            tag = tag_for(v0, omega)
            frames_dir = os.path.join(SWEEP_ROOT, tag, FRAMES_SUBDIR)
    ...
    fig.suptitle("Fig2-style phase diagram sweep — spring repulsion (last frame of each run)")
    fig.tight_layout()
...
def main():
    ...
    print(f"Launching {len(combos)} sims across {N_WORKERS} workers "
          f"(T={T_SIM}, N={sim.P.N}, L={sim.P.L}, k_rep={sim.P.k_rep}, mu={sim.P.mu})")
